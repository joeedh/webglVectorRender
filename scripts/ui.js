...
define([
  ...
  */
  ...
  class CurveTypeData {
    ...
    drawCurveLine(g, steps, w) {
      var df = 1/(steps-1), f=0.0;
      
      g.beginPath();
      for (var i=0; i<steps; i++, f += df) {
        var val = this.evaluate(f);
  ...
  class BSplineCurve extends CurveTypeData {
    ...
    loadJSON(obj) {
      ...
      if (obj.prescale !== undefined)
        this.prescale = obj.prescale;
      
      if (obj.postscale !== undefined)
        this.postscale = obj.postscale;
    ...
    regen_hermite(steps) {
      ...
      steps = steps == undefined ? 1024 : steps;
    ...
    regen_basis() {
      ...
      var steps = this.fastmode && this.deg > 2 ? 128 : 128;
    ...
    evaluate(t) {
      if (this.prescale !== undefined) {
        t *= this.prescale;
      }

      var a = this.points[0].rco, b = this.points[this.points.length-1].rco;
      
      if (t < a[0]) return a[1]*this.postscale;
      if (t > b[0]) return b[1]*this.postscale;

      if (this.points.length == 2) {
        t = (t - a[0]) / (b[0] - a[0]);
        return (a[1] + (b[1] - a[1])*t)*this.postscale;
      }
      ...
      return (table[i] + (table[i+3] - table[i])*s)*this.postscale; 
    }
    ...
    makeGUI(dom, gui, canvas, g, draw_transform) {
      ...
      let panel = gui; //XXX not working: ud.panel = gui.panel("Settings");
      ud.dslider = panel.slider(undefined, "Degree", this.deg, 
        0, 7, 1, true, true, (val) => {
        this.deg = val;
        this.update();
        this.redraw();
      ...
      ud.sslider = panel.slider(undefined, "PreScale", this.prescale,
        -1.0, 2.0, 0.0001, false, true, (val) => {
        this.prescale = val;
        this.update();
        this.redraw();
        this.doSave();
      ...
      ud.pslider = panel.slider(undefined, "PostScale", this.postscale,
        -1.0, 35.0, 0.0001, false, true, (val) => {
        this.postscale = val;
        this.update();
        this.redraw();
        this.doSave();
    ...
    drawCurveLine(g, steps, w) {
      var df = 1.0/(steps-1), f=0.0;
      let ps = this.prescale;
      let ps2 = this.postscale;
      
      //XXX hackish draw line without transformations
      this.prescale = this.postscale = 1.0;
  ...
  var CurveWidget = exports.CurveWidget = class CurveWidget {
    ...
    save() {
      if (typeof this.setting_id == "function") {
        this.setting_id();
      } else {
  ...
  var UI = exports.UI = class UI {
    ...
    */
    ...
    curve(id, name, default_preset) {
      id = typeof id == "string" ? this.storagePrefix + id : id;
