...
//  console.trace("seed called");
  ...
  var MinMax = exports.MinMax = exports.Class([
    function constructor(size) {
      size = size == undefined ? 3 : size;
      
      this.min = new Array(size);
      this.max = new Array(size);

      this._min = new Array(size);
      this._max = new Array(size);
    ...
    function reset() {
      for (var i=0; i<this.min.length; i++) {
        this._min[i] = 1e17, this._max[i] = -1e17;
        this.min[i] = this.max[i] = 0.0;
      }
    },
    
    function minmax(co) {
      for (var i=0; i<this.min.length; i++) {
        this.min[i] = this._min[i] = Math.min(this._min[i], co[i]);
        this.max[i] = this._max[i] = Math.max(this._max[i], co[i]);
      }
